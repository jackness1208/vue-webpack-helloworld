...
gulp.task('default', function(){
    console.log([
        ...
        '    ' + 'webpack            run webpack',
        '    ' + 'commit             pack project & commit to server',
        '    ' + 'all                init project',
        ''
    ].join(''));
});
        ...
        pathTrans = function(src){
            if(/(\.css|\.css\.map)$/.test(src)){
                return path.joinFormat(path.relative( 
                    path.join(__dirname, config.path.root), 
        ...
        new fn.Promise(function(next){
            ...
            for(var src in outRev){
                if(outRev.hasOwnProperty(src) && /css|js|map/.test(path.extname(src))){
                    fPath = path.join(__dirname, config.path.root, outRev[src]);
                    if(fs.existsSync(fPath)){
                        fs.writeFileSync(
                            path.join(__dirname, config.path.root, src), 
            ...
            if(gulp.env.ver == 'remote' && gulp.env.sub){
                ...
                fn.get(iConfig.revAddr, function(data){
                    console.log('================'.green);
                        ...
                        for(var src in revData){
                            if(revData.hasOwnProperty(src)){
                                if(revData[src] != outRev[src]){
                                    fPath = path.join(__dirname, config.path.root, revData[src]);
                                    if(fs.existsSync(fPath)){
                                        fs.writeFileSync(
                                            path.join(__dirname, config.path.root, outRev[src]), 
                                            fs.readFileSync(fPath)
                                        );
